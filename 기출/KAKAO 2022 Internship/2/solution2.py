...
def getMinimumMoves(firstQueue, secondQueue, target):
    ...
    for left in range(1, queueSize-1):
        # total = 0
        p1, p2 = left, queueSize
        previousMid = (p1 + p2) // 2
        total = sum(queue[left:previousMid])
        total += queue[left]
        while p1 <= p2 and p2 <= queueSize:
            mid = (p1 + p2) // 2
            if mid < previousMid:
                total -= sum(queue[mid:previousMid])
            else:
                total += sum(queue[previousMid:mid])
            previousMid = mid
            # total = sum(queue[left:mid])
            if total < target:
                p1 = mid + 1
            elif total > target:
                p2 = mid - 1
            else:
                minMoves = min(minMoves, calculateMoves(queue, left, mid))
                break
